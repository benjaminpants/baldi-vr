...
public class GameControllerScript : MonoBehaviour
{
	...
	private void Update()
	{
        Debug.Log(SteamVR_Input.GetFloat("ItemSelect", SteamVR_Input_Sources.LeftHand));
        if (!this.learningActive)
		{
			...
			if (SteamVR_Input.GetFloat("ItemSelect", SteamVR_Input_Sources.LeftHand) > 0f)
			{
        ...
        this.item_delay -= Time.unscaledDeltaTime;
	...
	{
		this.itemSelect.anchoredPosition = new Vector3((float)this.itemSelectOffset[this.itemSelected], 21.9f, 0f); //Moves the item selector background(the red rectangle)
        this.UpdateItemName();
	}
