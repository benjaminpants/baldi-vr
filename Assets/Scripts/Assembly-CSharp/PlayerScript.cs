...
public class PlayerScript : MonoBehaviour
{
    ...
    public Goto_Player_Object vr_offset;
    public GameObject r_control;
	...
	{
        this.playerRotation.eulerAngles = new Vector3(0f, (this.r_control.transform.rotation.eulerAngles.y), 0f);
        vr_offset.rotational_offset = new Vector3(0f,camera_offset,0f);
    }
	...
	private void PlayerMove()
	{
        ...
        camera_offset += SteamVR_Input.GetVector2("Orientate", SteamVR_Input_Sources.LeftHand).x * 5f;
        this.db = SteamVR_Input.GetVector2("Move",SteamVR_Input_Sources.RightHand).y;
        Debug.Log(SteamVR_Input.GetVector2("Orientate", SteamVR_Input_Sources.LeftHand).x);
		if (this.stamina > 0f)
		{
			if (SteamVR_Input.GetBooleanAction("Run",false).state)
			{
		...
		if (SteamVR_Input.GetVector2("Move", SteamVR_Input_Sources.RightHand).y > 0f)
		{
			a = base.transform.forward;
		}
		else if (SteamVR_Input.GetVector2("Move", SteamVR_Input_Sources.RightHand).y < 0f)
		{
			a = base.transform.forward * -1f;
		}
		if (SteamVR_Input.GetVector2("Move", SteamVR_Input_Sources.RightHand).x > 0f)
		{
			//b = base.transform.right;
		}
		else if (SteamVR_Input.GetVector2("Move", SteamVR_Input_Sources.RightHand).x < 0f)
		{
	...
	private void StaminaCheck()
	{
		...
		{
			if (SteamVR_Input.GetBooleanAction("Run", false).state & this.stamina > 0f)
			{
